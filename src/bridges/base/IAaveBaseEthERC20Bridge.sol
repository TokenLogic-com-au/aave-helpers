...
interface IAaveBaseEthERC20Bridge {
  /// @notice Returned when calling the contract from an invalid chain
  error InvalidChain();

  /// @notice Emitted when bridging a token from Base to Mainnet
  ...
  /// @notice Returns the Base Standard Bridge Address
  /// @return Address of bridge
  function L2_STANDARD_BRIDGE() external returns (address);

  /// @notice Bridges ERC20 token from Base to Ethereum Mainnet
