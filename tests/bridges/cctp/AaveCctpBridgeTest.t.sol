...
contract AaveCctpBridgeTestBase is Test {
  ...
  function setUp() public {
    ...
    MockMessageTransmitterV2 mockMessageTransmitter = new MockMessageTransmitterV2(
...
contract BridgeTest is AaveCctpBridgeTestBase {
  ...
  function test_successful_fastTransfer() public {
    ...
    assertEq(
      usdc.balanceOf(address(mockTokenMessenger)),
      AMOUNT,
      'TokenMessenger should have USDC'
    );
...
contract ViewFunctionsTest is AaveCctpBridgeTestBase {
  ...
  function test_getDestinationCollector() public view {
    assertEq(
      bridge.getDestinationCollector(CctpConstants.ARBITRUM_DOMAIN),
      _addressToBytes32(receiver)
    );
    assertEq(
      bridge.getDestinationCollector(CctpConstants.BASE_DOMAIN),
      _addressToBytes32(receiver)
    );
  }
}
...
contract SetDestinationCollectorTest is AaveCctpBridgeTestBase {
  ...
  function test_successful() public {
    ...
    assertEq(
      bridge.getDestinationCollector(CctpConstants.ARBITRUM_DOMAIN),
      _addressToBytes32(newCollector)
    );
  }
