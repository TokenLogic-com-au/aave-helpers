...
contract BridgeEthereumToArbitrumTest is AaveOFTBridgeForkTestBase {
  function test_bridge_happyPath() public {
    ...
    assertEq(
      IERC20(ETHEREUM_USDT).balanceOf(owner),
      0,
      'User should have no USDT left on ETH after bridging'
    );
  }
  ...
  function test_bridge_withMsgValue() public {
    ...
    assertEq(
      IERC20(ETHEREUM_USDT).balanceOf(owner),
      0,
      'User should have no USDT left after bridging'
    );
  }
...
contract RescuableTest is AaveOFTBridgeForkTestBase {
  ...
  function test_maxRescue_returnsMaxUint() public {
    ...
    assertEq(
      ...
      type(uint256).max,
      'maxRescue should return max uint256'
    );
    assertEq(
      mainnetBridge.maxRescue(address(0)),
      type(uint256).max,
      'maxRescue should return max uint256 for any address'
    );
  }
